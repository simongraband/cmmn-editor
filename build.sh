...
if [ "$copyBackend" == "true" ]; then
  ...
  inputGLSPServer=server/org.eclipse.emfcloud.cmmn.glsp/target/org.eclipse.emfcloud.cmmn.glsp-0.0.1-glsp.jar
  outputGLSPServer=client/cmmn-server/build
  echo "  $(date +"[%T.%3N]") Copy CMMNGLSPServer to '$outputGLSPServer'."
  rm -rf $outputGLSPServer && mkdir -p $outputGLSPServer && cp -rf $inputGLSPServer $outputGLSPServer

  inputCMMNMS=server/org.eclipse.emfcloud.cmmn.modelserver/target/org.eclipse.emfcloud.cmmn.modelserver-0.0.1-SNAPSHOT-standalone.jar
  outputCMMNMS=client/cmmn-server/build
  echo "  $(date +"[%T.%3N]") Copy CMMNModelServer to '$outputCMMNMS'."
  cp -rf $inputCMMNMS $outputCMMNMS
